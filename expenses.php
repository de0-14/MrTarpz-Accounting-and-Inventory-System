...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'add_expense') {
        ...
        // INTEGRATED FIX: Prepared statement handles the 'notes' column securely
        $stmt = $conn->prepare("INSERT INTO expenses (expense_date, category, description, amount, payment_method, reference, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssi", $expense_date, $category, $description, $amount, $payment_method, $reference, $notes, $user_id);
        
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Expense added successfully', 'expense_id' => $conn->insert_id]);
        } else {
    ...
    if ($_POST['action'] == 'update_expense') {
        ...
        // INTEGRATED FIX: Secure update with 'notes' column
        $stmt = $conn->prepare("UPDATE expenses SET expense_date = ?, category = ?, description = ?, amount = ?, payment_method = ?, reference = ?, notes = ? WHERE expense_id = ?");
        $stmt->bind_param("sssssssi", $expense_date, $category, $description, $amount, $payment_method, $reference, $notes, $expense_id);
...
$common_categories = [
    ...
    'Maintenance', 'Marketing', 'Transportation', 'Raw Materials', 
    'Printing Supplies', 'Packaging', 'Insurance', 'Taxes', 
    'Legal Fees', 'Consulting', 'Training', 'Travel', 'Meals', 
    'Software', 'Internet', 'Phone', 'Water', 'Electricity', 'Others'
];
?>
...
<head>
    ...
    <style>
        .expense-summary { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 15px; margin-bottom: 20px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; }
        ...
        .date-range input { padding: 8px 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 14px; }
        .expense-category-badge { padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 500; }
    </style>
</head>
<body>
        ...
        <div class="main-content">
            ...
            <div class="filters-bar">
                ...
                <div class="date-range">
                    <input type="date" id="fromDate"><span>to</span><input type="date" id="toDate">
                </div>
                
                <button class="btn btn-secondary" onclick="loadExpenses()"><i class="fas fa-sync-alt"></i> Refresh</button>
                <button class="btn btn-success" onclick="exportExpenses()"><i class="fas fa-download"></i> Export</button>
            </div>
    ...
    <script>function toggleSidebar() { document.querySelector('.sidebar').classList.toggle('open'); }</script>
</body>
</html>