...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'get_payments') {
        ...
        $sql = "SELECT p.*, o.order_id, o.total_amount, o.paid_amount, 
                ...
                WHEN o.total_amount = o.paid_amount THEN 'Paid' 
    ...
    if ($_POST['action'] == 'get_unpaid_orders') {
        $current_order = isset($_POST['order_id']) ? sanitize($_POST['order_id']) : null;
        ...
        if ($current_order) {
            $sql .= " OR o.order_id = '$current_order'";
        }
    ...
    if ($_POST['action'] == 'update_payment') {
        ...
        $reference = !empty($_POST['reference_number']) ? "'" . sanitize($_POST['reference_number']) . "'" : "NULL";
        $notes = sanitize($_POST['notes']);
        ...
        try {
            $payment_sql = "SELECT * FROM payments WHERE payment_id = '$payment_id'";
            $payment = $conn->query($payment_sql)->fetch_assoc();

            $order_id = $payment['order_id'];
            $old_amount = floatval($payment['amount']);
            ...
            $new_paid = $current_paid - $old_amount + $amount;

            if ($new_paid > $total + 0.01) {
                throw new Exception('Updated amount exceeds total');
            }
            ...
            $status = 'unpaid';
            if ($new_paid > 0 && $new_paid < $total) $status = 'partial';
            if (abs($new_paid - $total) < 0.01) $status = 'paid';
...
<body>
    ...
    <script>
                ...
                payments.forEach(function(p) {
                    ...
                    html += `
                        <tr>
                            ...
                            <td><span class="status-badge status-${formatMethod(p.payment_status)}">${p.payment_status}</span></td>
                            ...
                            <td class="actions">
                                ...
                                <button class="btn-icon" style="color: #10b981" onclick="editPayment(${p.payment_id})" title="Edit">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (selectedOrderId) {
                            $('#select_order').val(selectedOrderId);
                            updateOrderDetails();
                        }
                    }
                }
        ...
        function showRecordPaymentModal(orderId = null) {
            ...
            $('#select_order').prop('disabled', false);
        ...
        function recordPayment() {
            const editId = $('#paymentForm').data('edit-id');
            ...
            const data = {
                action: editId ? 'update_payment' : 'record_payment',
                payment_id: editId,
            ...
            const html = `
                <div style="text-align: center;">
                    ...
                    <div style="background: #f8fafc; padding: 15px; border-radius: 10px; margin: 20px 0;">
                        ...
                        <p><strong>Reference:</strong> ${cells.eq(7).text()}</p>
                        <p><strong>Notes:</strong> ${cells.eq(8).text()}</p>
                    </div>
                </div>
            `;
        ...
        function editPayment(paymentId) {
            const row = $(`button[onclick="editPayment(${paymentId})"]`).closest('tr');
            const cells = row.find('td');

            const orderId = cells.eq(2).text().replace('#', '').trim();
            const amount = cells.eq(4).text().replace('₱', '').trim();
            const methodText = cells.eq(5).text().trim();
            const reference = cells.eq(6).text().trim();
            const notes = cells.eq(7).text().trim();

            // ✅ THIS is the correct call (you forgot this)
            loadUnpaidOrders(orderId);

            // Fill form
            $('#amount').val(amount);
            $('#reference_number').val(reference === '-' ? '' : reference);
            $('#payment_notes').val(notes === '-' ? '' : notes);
            ...
            $('#payment_method').val(methodMap[methodText]);

            // Disable AFTER loading
            $('#select_order').prop('disabled', true);

            $('#paymentForm').data('edit-id', paymentId);
